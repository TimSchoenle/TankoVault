...
/// constant. A constant would be wrong twice over: it drifts silently the moment the shell is
/// edited (the only symptom is a browser quietly refusing to run the script), and the bytes
/// differ per checkout — the repository's `index.html` is CRLF on a Windows working copy while
/// the built artefact is LF, and a CSP hash covers the script text byte for byte.
///
