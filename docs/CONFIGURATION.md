...
| `TANKOVAULT_AUTH__STEP_UP_TTL_MINUTES` | `5` | How long a step-up elevation lasts before a sensitive action prompts for the second factor again. Longer means fewer prompts and a wider window in which a walked-away-from laptop is a standing elevation; shorter means a prompt per click. Elevations are revoked outright on a password change, a sign-out, and the removal of the factor that earned one, whatever this is set to. |
