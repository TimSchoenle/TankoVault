...
///
/// The **image name** is checked for a second reason, added when migration 0026 made pgvector a
    ...
    let Some((_, harness_image)) = const_str(&harness, "POSTGRES_IMAGE") else {
        anyhow::bail!(
            "repo-lint: {} declares no `const POSTGRES_IMAGE: &str = \"…\"`; nothing else \
             ...
             `postgres` cannot apply migration 0026",
            harness_path.display()
        );
    };
        ...
        findings.push(Finding {
            ...
            detail: format!(
                "tests run `{harness_image}`, production runs `{compose_image}` \
                 ({COMPOSE}:{compose_line}); migration 0026 needs `CREATE EXTENSION vector`, so \
