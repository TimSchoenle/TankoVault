...
# Full E2E stack (Postgres + Redis + NATS + TRAWL + all services + web frontend).
...
| `solver` | ✅ | `ChallengeSolver` trait, `detect_challenge` classifier, TRAWL back-end, fake solver for the swap test. |
...
| `challenge-solver` | ✅ | `POST /v1/solve` HTTP contract, TRAWL-backed default, config-selected back-end. |
