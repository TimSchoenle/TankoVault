//! Fixture tests for the shipped Madara provider presets (manhuaus, kunmanga). These
//! exercise the *actual* `presets::builtin()` config against markup trimmed from live
//! solver-fetched pages, so a wrong selector override fails here rather than in production.
//! Each site deviates from the Madara defaults in a few ways this test pins down.
...
impl Fetcher for SiteFetcher {
    async fn get(&self, req: FetchRequest) -> Result<FetchResponse, FetchError> {
        // Catalogue listings live under `/manga/page/`; anything else /manga/ is a series.
...
fn preset_adapter(
    ...
    let ctx = Ctx {
        ...
        fetcher: Arc::new(SiteFetcher { catalog, series }),
    };
    (adapter, ctx)
}
...
async fn kunmanga_catalog_excludes_ads_and_marks_next() {
    let (adapter, ctx) = preset_adapter("kunmanga", KUNMANGA_CATALOG, KUNMANGA_SERIES);
    let page = adapter.list_catalog(&ctx, 1).await.expect("catalog parses");
...
#[tokio::test]
async fn kunmanga_series_reads_div_chapters() {
    let (adapter, ctx) = preset_adapter("kunmanga", KUNMANGA_CATALOG, KUNMANGA_SERIES);
    let meta = adapter
    ...
    // Chapter rows are <div>, reached only via the container override.
    let chapters = adapter
        .fetch_chapters(&ctx, "/manga/its-just-business")
        .await
        .expect("chapters parse");
    assert_eq!(chapters.len(), 3);
    assert!(chapters.iter().any(|c| (c.number - 118.0).abs() < 1e-9));
    assert_eq!(chapters[0].path, "/manga/its-just-business/chapter-118");
}
