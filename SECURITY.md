...
## Known issues an operator must act on

These are tracked, not secret. They are here because a deployment is affected by them today and
no code change closes them:
...
- **`OP-1`/`OP-2` — credentials in git history.** An AniList client secret and a token-encryption
  key were committed and later removed. Removing a file does not un-publish it: rotate both, and
  re-encrypt or truncate `external_accounts` under the new key.

Both, with their full reasoning, are in `docs/audit/PROGRESS.md`.
