...
| ARCH-19 | `auth.rs` (676) and `admin/users.rs` (643) approaching god size | **PARTIAL** | `auth.rs` (743 by the time it was split) is done: `services/api/src/auth/{register,login,session,password,verification,validate}.rs`, one module per flow, largest 173 lines. The re-exports are globs on purpose — `#[utoipa::path]` generates a sibling `__path_<handler>` that `routes!(auth::login)` resolves alongside the handler, so naming the handlers individually compiles and then fails to route; the module docs say so. The split that earns its keep is `validate`: those validators are not the registration handler's private business (`patch_profile` and `admin::update_user` write the same columns, and the `@`-in-username rule going unenforced on one of those paths is exactly what SEC-9 was), and the module is `pub(crate)` re-exported to say that. The dummy-hash test moved to `login.rs`, next to the branch it protects, rather than staying with the validators. **`admin/users.rs` (613) is not done.** |
