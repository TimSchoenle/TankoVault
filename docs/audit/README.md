...
Everything here is either remotely exploitable or a red gate hiding other failures. Nothing in this phase is a refactor.

| # | Item | Source | Effort |
| --- | --- | --- | --- |
| 0.1 | Stop publishing `sync` (8083), `control-plane` (8081), `render`/`challenge-solver` (8084), Postgres, Redis, and NATS on the host in `deploy/docker-compose.yml`; move to an internal network | SEC-1, SEC-2 | S |
| 0.2 | Add service-to-service authentication on `sync`, and derive `user_id` from the authenticated principal rather than the request body | SEC-1 | 1d |
| 0.3 | Validate URLs in `render`/`challenge-solver` through the SSRF guard; reject non-http(s) schemes; require auth | SEC-2 | 0.5d |
| 0.4 | Fix the `html.rs:115` char-boundary panic (byte offset from `to_lowercase()` applied to the original string) — **verified crash**, reachable from provider chapter-anchor text | TEST F-01 | S |
| 0.5 | Fix the quadratic `collect_objects` in `json.rs:120-186` — **verified**: 600 KB input takes 29.5s; the 8 MiB body cap implies ~96 min CPU | TEST F-02 | S |
| 0.6 | Fix `rustfmt.toml:6` (drop the nightly-only `ignore`; fix the generator to emit newlines instead) and confirm the `lint` job — and its three dependent jobs — actually pass | OPS-2, OPS-3 | 0.5d |
| 0.7 | Make the SSRF guard load-bearing: call `resolve_checked`/`is_forbidden_ip` on the connector path, and validate `base_url` in `admin/providers.rs` | SEC-5 | 0.5d |
| 0.8 | Require a current-password check + re-verification + session revocation on `PATCH /v1/me/profile` email change; add an authenticated change-password endpoint | SEC-4 | 0.5d |
| 0.9 | Fix the `X-Forwarded-For` rate-limit bypass: read the right-most entry, not the left-most | SEC-3 | S |
