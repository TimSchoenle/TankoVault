//! Markdown rendered as `rsx!` nodes, never as an HTML string.
//!
//! # Why there is no HTML anywhere in this file
//!
//! Both callers render text this binary did not write — an operator's legal document, and the
//! release notes attached to a GitHub release. Rendering Markdown to an HTML string and
//! injecting it would need `dangerous_inner_html`, and would make their correctness depend on a
//! sanitiser being right about every input.
//!
//! So `pulldown-cmark` is used as a pull parser and its events are mapped onto `rsx!` nodes
//! directly. There is no HTML string at any point, which means the source has nothing to inject
//! *into*; a raw `<script>` arrives as `Event::Html` and is rendered as the literal text it is.
...
pub(crate) fn markdown(source: &str) -> Element {
