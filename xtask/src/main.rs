...
/// Prepended verbatim to `crates/api-client/src/lib.rs`, ahead of `rustfmt`'s output.
///
/// The crate root a generated file cannot write for itself. Both halves earn their line:
/// `rustdoc::broken_intra_doc_links` is denied estate-wide, and every `[`Link`]` span in this
/// file arrived inside an `OpenAPI` `description` naming an item in `services/api` — so without
/// the allow, the documentation job fails on prose that was reviewed somewhere else.
/// `crates/api-client/Cargo.toml` carries the matching `missing_docs` exemption, and
/// `crates/api-client/tests/workspace_lints.rs` is what keeps that one honest.
#[cfg(feature = "full")]
const CLIENT_HEADER: &str = concat!(
    "// Generated by xtask. DO NOT EDIT.\n",
    "//! The typed client for this deployment's own HTTP API, generated from `openapi.json`.\n",
    "//!\n",
    "//! Every doc comment below is an OpenAPI `description`, and each of those came from a `///`\n",
    "//! in `services/api`. That is where they are reviewed and where their links resolve; here\n",
    "//! the same text names items this crate does not contain.\n",
...
fn openapi(check: bool) -> anyhow::Result<()> {
    ...
    let rendered_client = CLIENT_HEADER.to_owned()
